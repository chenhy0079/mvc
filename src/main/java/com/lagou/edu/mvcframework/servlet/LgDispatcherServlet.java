...
//        method.invoke() //
        ...
        //权限拦截
        LagouSecurity annotationCollector = handler.getController().getClass().getAnnotation(LagouSecurity.class);
        if(null==annotationCollector){
            LagouSecurity annotationMethod = handler.getMethod().getAnnotation(LagouSecurity.class);
            if(null!=annotationMethod){
                String[] value = annotationMethod.value();
                boolean username = Arrays.asList(value).contains(req.getParameter("username"));
            ...
            String[] value = annotationCollector.value();
            String name = (String)req.getParameter("username");
            boolean username = Arrays.asList(value).contains(name);

            if(!username){
                LagouSecurity annotationMethod = handler.getMethod().getAnnotation(LagouSecurity.class);
                if(null!=annotationMethod){
                    String[] valueMe = annotationMethod.value();
                    boolean usernameMe = Arrays.asList(valueMe).contains(req.getParameter("username"));
        ...
        Object invoke = null;
        try {
            invoke = handler.getMethod().invoke(handler.getController(), paraValues);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
